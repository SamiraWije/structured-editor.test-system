  ...
  public Object lambda(Object x) throws JasymcaException, ParseException {
    switch (length(x)) {
      case 1 :
        return car(x);
      case 2 :
        return ((Algebraic) car(x)).add((Algebraic) car(cdr(x)));
    }
  ...
  public Object lambda(Object x) throws JasymcaException, ParseException {
    switch (length(x)) {
      case 1 :
        return car(x);
      case 2 :
        return ((Algebraic) car(x)).div((Algebraic) car(cdr(x)));
    }
    throw new ParseException("Wrong number of arguments for \"/\".");
  }
}
  ...
  Zahl f(Zahl x) throws JasymcaException {
    if (!x.integerq() || x.smaller(Zahl.ZERO))
      throw new JasymcaException(
          "Argument to factorial must be a positive integer.");
...
class FunctionDefine extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    if (fname == null || vars == null || code == null
        || !(fname instanceof String))
      throw new ParseException("Wrong function definition.");
    env.putValue((String) fname, new UserFunction((String) fname, vars, code,
        env));
...
public abstract class Lambda extends Lisp {
  ...
  static Vector<Object> getArgs(Object args) throws ParseException {
...
abstract class LambdaAlgebraic extends Lambda {
  ...
  public Algebraic integrate(Algebraic arg, Variable x) throws JasymcaException {
    if (!arg.depends(x))
      throw new JasymcaException(
          "Expression in function does not depend on Variable.");
    ...
    try {
      ...
      // * dx/dz <----> /coef[1]
      return ((Algebraic) Jasymca.evalPrefix(prefix, true, env))
          .div(((Polynomial) arg).coef[1]);
    } catch (Exception e) {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    if (arg instanceof Unexakt)
      return f((Zahl) arg);
    return FunctionVariable.create(getClass().getName().substring(
        "jasymca.Lambda".length()).toLowerCase(Locale.US), arg);
  }
}
...
class LambdaCFS extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    Vector<Object> args = getArgs(car(x));
    Algebraic y = ((Algebraic) Jasymca.evalPrefix(args.elementAt(0), true, env))
        .rat();
...
class LambdaDIFF extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    Object f = Jasymca.evalPrefix(args.elementAt(0), true, env); // Function to
                                                                 // differentiate
...
class LambdaEXPAND extends Lambda {
  @Override
  public Object lambda(Object x) throws ParseException, JasymcaException {
    return new SqrtExpand().f_exakt((Algebraic) Jasymca.evalPrefix(car(x),
        true, env));
  }
}
...
class LambdaGCD extends Lambda {
  Algebraic gcd(Algebraic x, Algebraic y) throws JasymcaException {
    ...
    if (x instanceof Polynomial) {
      if (y instanceof Polynomial) { // poly_gcd * coef_gcd
        ...
        for (Algebraic element : ((Polynomial) y).coef)
          gcd = gcd(gcd, element);
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    Algebraic gcd = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(0), true, env);
    for (int i = 1; i < args.size(); i++)
      gcd = gcd(gcd, (Algebraic) Jasymca.evalPrefix(args.elementAt(i), true,
          env));
    return gcd;
  }
}
...
class LambdaLOADFILE extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    try {
      ...
      throw new JasymcaException("Could not read from " + filename + " :"
          + e.toString());
    }
  }
...
class LambdaLSUM extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    Jasymca.evalPrefix(args.elementAt(0), true, env);
    Algebraic ind = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(1), true, env);
    Algebraic list = (Algebraic) Jasymca.evalPrefix(args.elementAt(2), true,
        env);
    if (!(ind instanceof Polynomial) || !(list instanceof Vektor))
      throw new ParseException("Usage: LSUM (exp, ind, list)");
    Algebraic sum = Zahl.ZERO;
    for (Algebraic element : ((Vektor) list).coord)
      sum = sum.add(element);
    return sum;
  }
}
...
class LambdaSAVE extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    try {
      ...
      for (int i = 1; i < args.size(); i++) {
        ...
        if ("ALL".equalsIgnoreCase(var)) {
          ...
          while (en.hasMoreElements()) {
            ...
            if (!"pi".equalsIgnoreCase((String) key)) { // Would be reread as
                                                        // variable
      ...
      throw new JasymcaException("Could not write to " + filename + " :"
          + e.toString());
    }
  }
}
    ...
    for (int i = 0; i < fs.length; i++) {
      ...
      if (fs[i] != null)
        if (i == 0)
          ...
          factor = list(cons("^", cons(fs[i], list(new Unexakt(i + 1)))));
      if (factor != null)
        if (result == null)
          result = factor;
        else
...
class LambdaSQRT extends LambdaAlgebraic {
  ...
  Algebraic fzexakt(Zahl x) throws JasymcaException {
    if (x instanceof Exakt && !x.komplexq()) {
      ...
      BigInteger br[] = {BigInteger.valueOf(a0), BigInteger.valueOf(b0 * b1)};
      Exakt r = new Exakt(br);
      a0 = a1 * b1;
      if (a0 == 1L)
        return r;
      return r.mult(new Polynomial(new FunctionVariable("sqrt", new Exakt(
          BigInteger.valueOf(a0)), this)));
    }
    return null;
  }
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    if (args.size() != 3)
      throw new ParseException(
          "Usage: SUBST (a, b, c), substitutes a for b in c");
...
class LambdaSUM extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    Algebraic exp = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(0), true, env);
    Algebraic ind = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(1), true, env);
    ...
    if (!(ind instanceof Polynomial) || !(lo instanceof Zahl)
        || !(hi instanceof Zahl))
      throw new ParseException("Usage: SUM (exp, ind, lo, hi)");
...
class LambdaTAYLOR extends Lambda {
  ...
  public Object lambda(Object x) throws ParseException, JasymcaException {
    ...
    Algebraic exp = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(0), true, env);
    Algebraic var = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(1), true, env);
    Algebraic pt = (Algebraic) Jasymca.evalPrefix(args.elementAt(2), true, env);
    Algebraic pow = (Algebraic) Jasymca
        .evalPrefix(args.elementAt(3), true, env);
    if (!(var instanceof Polynomial) || !(pow instanceof Zahl)
        || !((Zahl) pow).integerq())
      throw new ParseException("Usage: TAYLOR (exp, var, pt, pow)");
    Variable v = ((Polynomial) var).var;
  ...
  public Object lambda(Object x) throws JasymcaException, ParseException {
    switch (length(x)) {
      case 1 :
        return car(x);
      case 2 :
        return ((Algebraic) car(x)).mult((Algebraic) car(cdr(x)));
    }
    throw new ParseException("Wrong number of arguments for \"*\".");
  }
}
...
class Sub extends LambdaAlgebraic {
  ...
  public Object lambda(Object x) throws JasymcaException, ParseException {
    switch (length(x)) {
      case 1 :
        return ((Algebraic) car(x)).mult(Zahl.MINUS);
      case 2 :
        return ((Algebraic) car(x)).sub((Algebraic) car(cdr(x)));
    }
    throw new ParseException("Wrong number of arguments for \"-\".");
  }
}
...
class UserFunction extends LambdaAlgebraic {
  ...
  public UserFunction(String fname, Object var, Object code, Environment env)
    ...
    if (var instanceof String) {
      ...
      this.var[0] = new SimpleVariable((String) var + toString());
      body = body.value(new SimpleVariable((String) var), new Polynomial(
          this.var[0]));
    } else { // Multivariate function
      Vector<Object> v = getArgs(var);
      if (v.size() == 0)
      ...
      for (int i = 0; i < this.var.length; i++) {
        ...
        this.var[i] = new SimpleVariable((String) s + toString());
        body = body.value(new SimpleVariable((String) s), new Polynomial(
            this.var[i]));
      }
    }
    this.fname = fname;
    Lambda.env = env; // changed to static reference Michael Borcherds
                      // 2008-04-06
  }
