...
public class SingleArityBranchingUserFunction extends Evaluator {
    ...
    protected List<FunctionParameter> iParameters = new ArrayList<FunctionParameter>(); //CArrayGrower<FunctionParameter>

    /// List of rules, sorted on precedence.
    protected List<Branch> iBranchRules = new ArrayList<Branch>();//CDeletingArrayGrower<BranchRuleBase*>
...
/// evaluate the function with some arguments.
    ...
    public void evaluate(Environment aEnvironment, ConsPointer aResult, ConsPointer aArgumentsPointer) throws Exception {
        ...
        try {
            ...
            for (parameterIndex = 0; parameterIndex < arity; parameterIndex++) {
                String variableName = iParameters.get(parameterIndex).iParameter;
            ...
            for (parameterIndex = 0; parameterIndex < numberOfRules; parameterIndex++) {
                Branch thisRule = iBranchRules.get(parameterIndex);
                ...
                while (thisRule != iBranchRules.get(parameterIndex) && parameterIndex > 0) {
                    parameterIndex--;
                }
            }//end for.
    ...
    protected ConsPointer[] evaluateArguments(Environment aEnvironment, ConsPointer aArgumentsPointer) throws Exception {
        ...
        for (parameterIndex = 0; parameterIndex < arity; parameterIndex++) {
            ...
            if (iParameters.get(parameterIndex).iHold) {
    ...
    void insertRule(int aPrecedence, Branch newRule) {
        ...
        if (high > 0) {
            if (iBranchRules.get(0).getPrecedence() > aPrecedence) {
                mid = 0;
                // Insert it
                iBranchRules.add(mid, newRule);
                return;
            }
            if (iBranchRules.get(high - 1).getPrecedence() < aPrecedence) {
                mid = high;
                // Insert it
                iBranchRules.add(mid, newRule);
                return;
        ...
        for (;;) {
            ...
            if (iBranchRules.get(mid).getPrecedence() > aPrecedence) {
                high = mid;
            } else if (iBranchRules.get(mid).getPrecedence() < aPrecedence) {
                low = (++mid);
            } else {
                // Insert it
                iBranchRules.add(mid, newRule);
    ...
    public Iterator<Branch> getRules() {
        return iBranchRules.iterator();
    }

    public Iterator<FunctionParameter> getParameters() {
