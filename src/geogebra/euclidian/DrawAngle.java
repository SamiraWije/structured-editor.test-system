...
public class DrawAngle extends Drawable {
  ...
  private final Arc2D.Double drawArc = new Arc2D.Double();
  private final GeneralPath polygon = new GeneralPath(); // Michael Borcherds
  // 2007-11-19
  private Ellipse2D.Double dot90degree;
  private Shape shape;
  private final double m[] = new double[2];
  private final double coords[] = new double[2];
  private final double[] firstVec = new double[2];
  ...
  private final Arc2D.Double decoArc = new Arc2D.Double();
  private Line2D.Double[] tick;
  private final double[] angleTick = new double[2];
  ...
  protected DrawAngle(EuclidianView view, GeoAngle angle) {
    this.view = view;
  ...
  final public void draw(Graphics2D g2) {
    if (isVisible) {
      ...
      if (show90degrees)
        ...
        // if we don't have a special 90 degrees appearance we might need to
        // draw
        // other decorations
    ...
    if (show90degrees)
        ...
        case EuclidianView.RIGHT_ANGLE_STYLE_SQUARE :
          ...
          square.lineTo((float) (coords[0] + length * Math.cos(angSt)),
              (float) (coords[1] - length * Math.sin(angSt)
                  * view.getScaleRatio()));
          square
              .lineTo((float) (coords[0] + angle.arcSize
                  * Math.cos(angSt + Kernel.PI_HALF / 2)),
                  (float) (coords[1] - angle.arcSize
                      * Math.sin(angSt + Kernel.PI_HALF / 2)
                      * view.getScaleRatio()));
          square.lineTo((float) (coords[0] + length
              * Math.cos(angSt + Kernel.PI_HALF)), (float) (coords[1] - length
              * Math.sin(angSt + Kernel.PI_HALF) * view.getScaleRatio()));
      ...
      switch (geo.decorationType) {
        ...
        case GeoElement.DECORATION_ANGLE_ARROW_CLOCKWISE :
          ...
          ;
          p2[1] = p1[1] + (1 * n[1] + 3 * v[1]) * size * view.invYscale;
          ; // arrow end 1

          p3[0] = p1[0] + (-1 * n[0] + 3 * v[0]) * size * view.invXscale;
          ;
          p3[1] = p1[1] + (-1 * n[1] + 3 * v[1]) * size * view.invYscale;
          ; // arrow end 2
