...
public class Polynomial extends Algebraic {
  protected Algebraic[] coef = null;
  protected Variable var = null;

  // Polynomial with main-est Variable
  protected static Polynomial top = new Polynomial(SimpleVariable.top);
  ...
  protected Polynomial(Variable var) {
    coef = new Zahl[2];
    coef[0] = Zahl.ZERO;
    coef[1] = Zahl.ONE;
    this.var = var;
  ...
  protected Polynomial(Variable var, Algebraic[] coef) {
    this.var = var;
    this.coef = norm(coef);
  }
  ...
  // Use only for (a) constant coefficients and (b) squarefree function (c)
  // monic polynomial
  // Adapted from c-code by C. Bond (1991)
  protected Vektor bairstow() throws JasymcaException {
    ...
    for (int i = 0; i < coef.length; i++) {
      ...
      if (!(cf instanceof Zahl) || ((Zahl) cf).komplexq())
        throw new JasymcaException(
            "Bairstow requires constant real coefficients.");
  ...
  protected Algebraic[] clone(Algebraic[] x) {
  ...
  public boolean coefdepend(Variable var) {
    for (Algebraic element : coef)
      if (element.depends(var))
        return true;
    return false;
  }

  protected Algebraic coefficient(int i) {
    if (i < coef.length)
      return coef[i];
    return Zahl.ZERO;
  }
  ...
  protected boolean constcoef(Variable var) {
  ...
  boolean dependsmult(Variable var) throws JasymcaException {
    ...
    for (Algebraic element : coef)
      if (element instanceof Polynomial
          && ((Polynomial) element).dependsmult(var))
        return true;
    return false;
  }

  // List of variables, which depend on var
  protected Vector<Object> depvars(Variable var) throws JasymcaException {
    Vector<Object> r = new Vector<Object>();
    if (!this.var.deriv(var).equals(Zahl.ZERO))
      r.addElement(this.var);
    for (Algebraic element : coef)
      if (element instanceof Polynomial) {
        Vector<Object> c = ((Polynomial) element).depvars(var);
        if (c.size() > 0)
  ...
  @Override
  public Algebraic[] div(Algebraic q1, Algebraic[] result)
    ...
    if (!(q1 instanceof Zahl))
      throw new RuntimeException("div routine not implemented for" + this + "/"
          + q1);
    Algebraic[] csum = clone(coef);
  ...
  public Algebraic integrate(Variable var) throws JasymcaException {
    ...
    for (int i = 1; i < coef.length; i++)
      if (!coef[i].depends(var))
        if (var.equals(this.var))
          // c*x^n -->1/(n+1)*x^(n+1)
          in = in.add(coef[i].mult(new Polynomial(var).pow_n(i + 1).div(
              new Unexakt(i + 1))));
        else if (this.var instanceof FunctionVariable
            && ((FunctionVariable) this.var).arg.depends(var))
          // f(x)
          if (i == 1)
            in = in.add(((FunctionVariable) this.var).integrate(var).mult(
                coef[1]));
          // (f(x))^2, (f(x))^3 etc
          ...
          // Constant: c --> c*x
          in = in.add(coef[i].mult(new Polynomial(var).mult(new Polynomial(
              this.var).pow_n(i))));
        ...
        throw new JasymcaException("Integral not supported.");
          ...
          && ((FunctionVariable) this.var).arg.depends(var)) {
        if (i == 1 && coef[i] instanceof Polynomial
            && ((Polynomial) coef[i]).var.equals(var)) {
          // poly(x)*f(x)
          ...
          for (Algebraic element : ((Polynomial) coef[i]).coef)
            if (element.depends(var))
              throw new JasymcaException(
                  "Function not supported by this method");
          ...
          try {
            ...
            Algebraic f = new Polynomial(this.var);
        ...
        // mainvar independend of var, treat as constant and integrate coef
        in = in.add(coef[i].integrate(var).mult(
            new Polynomial(this.var).pow_n(i)));
    if (coef.length > 0)
  ...
  public Algebraic map(LambdaAlgebraic f) throws JasymcaException {
    Algebraic x = var instanceof SimpleVariable
        ? new Polynomial(var)
        : FunctionVariable.create(((FunctionVariable) var).fname, f
            .f_exakt(((FunctionVariable) var).arg));
    Algebraic r = Zahl.ZERO;
    for (int i = coef.length - 1; i > 0; i--)
  ...
  protected Polynomial monic() throws JasymcaException {
    Algebraic cm = coef[coef.length - 1];
    if (cm.equals(Zahl.ONE))
      return this;
    if (cm.equals(Zahl.ZERO) || !(cm instanceof Zahl))
      throw new JasymcaException(
          "Ill conditioned polynomial: main coefficient Zero or not number");
      ...
      if (var.equals(((Polynomial) p).var)) {
        ...
        for (int i = 0; i < coef.length; i++)
          for (int k = 0; k < ((Polynomial) p).coef.length; k++)
            cprod[i + k] = cprod[i + k].add(coef[i]
                .mult(((Polynomial) p).coef[k]));
  ...
  public boolean rational(Variable var) {
    ...
    for (int i = 0; i < coef.length; i++)
      if (coef[i] instanceof Polynomial
          && !((Polynomial) coef[i]).rational(var))
        return false;
    return true;
  }
  ...
  public Algebraic reduce() throws JasymcaException {
    ...
    // Rebuild Polynomial
    /*
     * Algebraic x = var.reduce(); Algebraic cn[] = new Algebraic[coef.length];
     * Algebraic r = Zahl.ZERO; for(int i=0; i<cn.length; i++) r =
     * r.add(coef[i].reduce()).mult(x); if(coef.lenth>0) r =
     * r.add(coef[0].reduce()); return r;
     */
  }
  ...
  protected Vektor solve(Variable var) throws JasymcaException {
    ...
    for (int i = 0; i < n; i++)
      if (factors[i] instanceof Polynomial) {
        ...
        for (Algebraic element : sol.coord)
  ...
  public Vektor solvepoly() throws JasymcaException {
    ...
    switch (degree()) {
      ...
      case 2 :
        ...
        break;
      /*
       * case 3: Algebraic a = r.coef[2]; Algebraic b = r.coef[1]; Algebraic c =
       * r.coef[0]; Zahl drei = new Exakt(3.0); // p=(3b-a^2)/3 p =
       * drei.mult(b).sub(a.mult(a)).div(drei); // q = c + 2a^3/27 -ab/3 q =
       * c.add(Zahl.TWO.mult(a.pow_n(3)).div(new Exakt(27.))).
       * sub(a.mult(b).div(drei)); // D^2 = (p/3)^3 + (q/2)^2 Algebraic D = new
       * Polynomial(new FunctionVariable("sqrt",
       * p.div(drei).pow_n(3).add(q.div(Zahl.TWO).pow_n(2)))); Algebraic u = new
       * Polynomial(new FunctionVariable("csqrt",
       * Zahl.MINUS.mult(q).div(Zahl.TWO).add(D))); Algebraic v = new
       * Polynomial(new FunctionVariable("csqrt",
       * Zahl.MINUS.mult(q).div(Zahl.TWO).sub(D))); Algebraic r0 =
       * Zahl.MINUS.mult(a).div(drei).add(u).add(v); Algebraic r1 =
       * Zahl.MINUS.mult(a).div(drei).sub(u.add(v).div(Zahl.TWO)); Algebraic w =
       * new Unexakt(Math.sqrt(3.)).mult(u.sub(v)).
       * div(Zahl.TWO).mult(Zahl.IONE); Algebraic r2 = r1.sub(w); r1 =
       * r1.add(w); s.addElement(r0); s.addElement(r1); s.addElement(r2); break;
       */
      default :
  ...
  protected Algebraic[] square_free_dec(Variable var) throws JasymcaException {
    if (!dependsmult(var))
      return null;
  ...
  public String toString() {
    ...
    for (int i = 0; i < x.size(); i++) {
      s += x.elementAt(i);
      if (i < x.size() - 1 && !(x.elementAt(i + 1).charAt(0) == '-'))
        s += "+";
    }
    if (x.size() > 1)
      s += ")";
  ...
  protected Algebraic value(Algebraic x) throws JasymcaException {
    return value(var, x);
  }
