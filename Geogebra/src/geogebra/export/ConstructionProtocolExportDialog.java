...
public class ConstructionProtocolExportDialog extends JDialog
  ...
  public ConstructionProtocolExportDialog(ConstructionProtocol prot) {
    ...
    System.out.println("ConstructionProtocolExportDialog - конструктор");
    this.prot = prot;
  ...
  private void centerOnScreen() {
    System.out.println("center on screen");
    pack();
   ...
   *          : states whether a picture of the drawing pad should be exportet
   *          with the html output file
   * @param includeAlgebraPicture
   *          : states whether a picture of the algebraWindow should be exportet
   *          with the html output file
   */
  private void exportHTML(boolean includePicture, int width,
    ...
    try {
      ...
      System.out.println("save image to PNG file");
      if (img != null) {
        pngFile = Application.addExtension(Application.removeExtension(file),
            "png");
        ImageIO.write(img, "png", pngFile);
      }

      System.out.println("write html string to file");
  ...
  private BufferedImage getCenterPanelImage() {
    ...
    BufferedImage img = new BufferedImage(centPanel.getWidth(), centPanel
  ...
  private void initGUI() {
    setResizable(false);
    setTitle(geogebra.Menu.Export + ": " + Plain.ConstructionProtocol + " ("
    ...
    cbDrawingPadPicture = new JCheckBox(Plain.InsertPictureOfConstruction);
    cbDrawingPadPicture.setSelected(true);
    cbScreenshotPicture = new JCheckBox(
        Plain.InsertPictureOfAlgebraAndConstruction);
    ...
    cbColor = new JCheckBox(Plain.ColorfulConstructionProtocol);
      ...
      public void actionPerformed(ActionEvent e) {
        Thread runner = new Thread() {
          ...
          public void run() {
            ...
            if (kernelChanged)
              app.storeUndoInfo();
            exportHTML(cbDrawingPadPicture.isSelected(), sizePanel
                .getSelectedWidth(), cbScreenshotPicture.isSelected(), cbColor
                .isSelected());
          }
        };
        runner.start();
