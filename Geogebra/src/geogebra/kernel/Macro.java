...
public class Macro {
  ...
  public static void addDependentElement(GeoElement geo, TreeSet consElementSet) {
    ...
    if (algo.isInConstructionList()) {
      ...
      for (GeoElement element : algoOutput)
        consElementSet.add(element);
    } else
  ...
  /**
   * Adds the geo, its parent algorithm and all input of the parent algorithm to
   * the consElementSet. This is used for e.g. a segment that is used as an
  ...
  public static void addSpecialInputElement(GeoElement geo,
      TreeSet<ConstructionElement> consElementSet) {
    // add geo
    consElementSet.add(geo);
    ...
    if (algo != null && algo.isInConstructionList()) {
      ...
      for (GeoElement element : algoInput)
        if (element.isLabelSet())
          consElementSet.add(element);
    }
  }
  /**
  ...
  private final LinkedList<AlgoMacro> usingAlgos = new LinkedList<AlgoMacro>();
   ...
   * input and output objects within this construction are given by inputLabels
   * and outputLabels
   * 
   * @param macroXML
   */
  ...
  /**
   * Команда, которая вызывает этот макрос
   * 
   * @return
   */
  ...
  /**
   * Returns the types of input objects of the default macro construction. This
   * can be used to check whether a given GeoElement array can be used as input
   * for this macro.
   */
  ...
  /**
   * Returns XML representation of this macro for saving in a ggb file.
   */
  ...
  public void initMacro(Construction macroCons, String[] inputLabels,
      String[] outputLabels) {
    this.macroCons = macroCons;
    // this.macroConsXML = macroCons.getConstructionXML();
    macroInputLabels = inputLabels;
    macroOutputLabels = outputLabels;
  ...
  private void initMacro(GeoElement[] input, GeoElement[] output)
    ...
    for (GeoElement element : output) {
      ...
      for (int k = 0; k < input.length; k++) {
        boolean dependencyFound = element.isChildOf(input[k]);
        if (dependencyFound) {
          dependsOnInput = true;
          inputNeeded[k] = true;
        }
      ...
      if (!dependsOnInput)
        throw new Exception(kernel.getApplication().getError(
            "Tool.OutputNotDependent")
    ...
    for (int i = 0; i < input.length; i++) {
      ...
      // we handle some special cases for input types like segment, polygons,
      // etc.
      switch (input[i].getGeoClassType()) {
        ...
        default :
          ...
          // make sure we don't have any parent algorithms of input[i] in our
          // construction
