 ...
 */
class SymbolLoader {
  private final ImageProducer[] imageSources = {null, null, null, null, null};
  private final String[] fontsizes = {"8", "10", "12", "14", "18"};
  private final Hashtable<String, Rectangle> fontdesH = new Hashtable<String, Rectangle>(
      189);
  ...
  public ImageProducer getBigImage(boolean appletB, boolean beanB,
      String filenameS, Applet app) {
    ...
    if (imageSource == null) { // Fonts are not local
      ...
      // Try loading external Font files in component/applet/bean specific
      // manner
      if (!appletB & !beanB)
        // component code
        imageSource = Toolkit.getDefaultToolkit().getImage(filenameS)
            .getSource();
      else if (appletB)
        // applet code
  ...
  public Image getImage(boolean appletB, boolean beanB, String filenameS,
    ...
    for (boolean loop = true; loop;) {
      if (fontsizes[++k].equals(fontSize + ""))
        loop = false;
      if (k == 4)
        loop = false;
    }
    ...
    if (imageSources[k] == null) {
      // original:
      // imageSources[k]=getBigImage(appletB, beanB, "Fonts"+fontsize+".gif",
      // app);
      // String desname = "Des"+fontsize+".gif";

      imageSources[k] = getBigImage(appletB, beanB,
          "Fonts" + fontSize + ".gif", app);
      String desname = "Des" + fontSize + ".gif";
      ...
      try {
        if (kLocalFonts) {
          getClass().getResourceAsStream(desname);
          // Application.debug("ip");
          istream = new BufferedInputStream(getClass().getResourceAsStream(
              desname));
          // Application.debug("nlocal");
        } else // Try loading external Font files in component/applet/bean
        // specific manner
        if (!appletB & !beanB)
          // component code
          ...
          // applet code
          istream = new BufferedInputStream(new URL(app.getCodeBase(), desname)
              .openStream());
        // Application.debug("file");
          ...
          // beanB==true
          try {
            istream = new BufferedInputStream(getClass().getResource(desname)
                .openStream());
          } catch (Exception ex) {
          }
        ...
        for (int i = 0; i < len; i++) {
          ...
          fontdesH
              .put(fontSize + ft, new Rectangle((Rectangle) p.readObject()));
        }
        istream.close();
      } catch (Exception exf) {
    ...
    Image image = null;
    if (imageSources[k] != null) {
      Rectangle r = fontdesH.get(fontSize + fn);
      image = Toolkit.getDefaultToolkit().createImage(
          new FilteredImageSource(new FilteredImageSource(imageSources[k],
              new CropImageFilter(r.x, r.y, r.width, r.height)),
              new ColorMaskFilter(g.getColor())));
    }
    return image;
  } // end getImage
  ...
  ImageProducer getLocalImageSource(String resourceName) {
    ...
    try {
      ...
      InputStream imageStream = SymbolLoader.class
      ...
      while (justRead != -1) {
        ...
        if (justRead != -1) { // didn't get all the bytes
          ...
          // Application.debug("+"+numBytes);
          if (totalBytes > imageBytes.length) { // haven't yet allocated enough
            // space
            byte[] tempImageBytes = imageBytes.clone();
