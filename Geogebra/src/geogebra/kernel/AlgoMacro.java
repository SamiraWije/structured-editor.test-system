...
//		}
	...
	private void initConic(GeoConic macroConic, GeoConic conic) {
		ArrayList<GeoElement> macroPoints = macroConic.getPointsOnConic();
		if (macroPoints == null) return;
		
		int size = macroPoints.size();
		ArrayList<GeoElement> points = new ArrayList<GeoElement>(size);		
		for (int i=0; i < size; i++) {
			points.add(getAlgoGeo(macroPoints.get(i)));
		}
