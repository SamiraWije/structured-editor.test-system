...
public class Exakt extends Zahl{
	...
	private void cfsd(Vector<Double> a, double[] y){
		int i=a.size()-1;
		double N=a.elementAt(i).doubleValue(), Z=1., N1;
		i--;
		while(i>=0){
			N1 = a.elementAt(i).doubleValue()*N+Z;
			Z = N;
			N = N1;
			i--;
		}
	...
	private BigInteger[] cfs(Vector<BigInteger> a) throws JasymcaException{
		int i=a.size()-1;
		BigInteger N=a.elementAt(i), Z=BigInteger.ONE, N1;
		i--;
		while(i>=0){
			N1 = a.elementAt(i).multiply(N).add(Z);
			Z = N;
			N = N1;
			i--;
		}
