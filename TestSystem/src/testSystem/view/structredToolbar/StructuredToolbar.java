...
package testSystem.view.structredToolbar;

import geogebra.euclidian.EuclidianView;
import geogebra.gui.MySmallJButton;
import geogebra.gui.toolbar.ModeToggleButtonGroup;
import geogebra.gui.toolbar.ModeToggleMenu;
...
public class StructuredToolbar extends JPanel implements ComponentListener {

  private static final long serialVersionUID = 8983050697241236851L;

  protected static final Integer TOOLBAR_SEPARATOR = new Integer(-1);
   ...
   */
  public static Vector<Serializable> createToolBarVec(String strToolBar) {
    String[] tokens = strToolBar.split(" ");
    Vector<Serializable> toolbar = new Vector<Serializable>();
    Vector<Integer> menu = new Vector<Integer>();

    for (String token : tokens)
      if (token.equals("|")) { // start new menu
        if (menu.size() > 0)
          toolbar.add(menu);
        menu = new Vector<Integer>();
      } else if (token.equals("||")) { // separator between menus
        if (menu.size() > 0)
          toolbar.add(menu);
        ...
        try {
          if (token.length() > 0) {
            int mode = Integer.parseInt(token);
  ...
  public static String getDefaultToolbarStringNoMacros() {
    ...
    sb.append(" ");
    sb.append(EuclidianView.MODE_SCRIPT_ACTION);
    sb.append(" ");
  ...
  /**
   * Creates a panel for the application's toolbar. Note: call initToolbar() to
   * fill the panel.
   */
  public StructuredToolbar(Application app) {
    this.app = app;
  ...
  private void addCustomModesToToolbar(JToolBar tb, ModeToggleButtonGroup bg) {
    Vector<Serializable> toolbarVec;
    try {
  ...
  public void initToolbar() {
    ...
    if (app.isUndoActive()) {
      ...
      // undo button
      MySmallJButton button = new MySmallJButton(app.getGuiManager()
          .getUndoAction(), 7);
      String text = geogebra.Menu.Undo;
      button.setText(null);
  ...
  /**
   * Returns mode text and toolbar help as html text with line breaks to fit in
   * the given panel.
   */
  private String wrappedModeText(String modeName, String helpText, JPanel panel) {
    ...
    int maxLines = Math.min(2, Math.round(panel.getHeight()
        / (float) fm.getHeight()));
