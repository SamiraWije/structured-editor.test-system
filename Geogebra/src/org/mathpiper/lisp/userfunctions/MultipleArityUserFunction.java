...
public class MultipleArityUserFunction
{
	...
	List<SingleArityBranchingUserFunction> iFunctions = new ArrayList<SingleArityBranchingUserFunction>();//
	...
	public SingleArityBranchingUserFunction getUserFunction(int aArity) throws Exception
	{
		...
		for (ruleIndex =0; ruleIndex<numberOfRules; ruleIndex++)
		{
			...
			if (iFunctions.get(ruleIndex).isArity(aArity))
			{
				return iFunctions.get(ruleIndex);
			}
		}
	...
	public void holdArgument(String aVariable) throws Exception
	{
		...
		{
			LispError.lispAssert(iFunctions.get(ruleIndex ) != null);
			iFunctions.get(ruleIndex ).holdArgument(aVariable);
		}
	}
	...
	public  void addRulebaseEntry(SingleArityBranchingUserFunction aNewFunction) throws Exception
	{
		...
		for (ruleIndex=0; ruleIndex<numberOfRules; ruleIndex++)
		{
			...
			LispError.check(!iFunctions.get(ruleIndex).isArity(aNewFunction.arity()),LispError.ARITY_ALREADY_DEFINED);
			LispError.check(!aNewFunction.isArity(iFunctions.get(ruleIndex).arity()),LispError.ARITY_ALREADY_DEFINED);
		}
    ...
    public Iterator<SingleArityBranchingUserFunction> getFunctions()
    {
