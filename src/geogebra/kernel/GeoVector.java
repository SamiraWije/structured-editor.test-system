...
implements Path, VectorValue, Locateable, Rotateable {
	...
	private HashSet<GeoPointInterface> waitingPointSet;
    ...
    public void setStartPoint(GeoPoint p) throws CircularDefinitionException {  
		...
		if (waitingForStartPoint) {
			...
			if (waitingPointSet != null) {
				...
				GeoPoint P;
				Iterator<GeoPointInterface> it = waitingPointSet.iterator();
				while (it.hasNext()) {
					P = (GeoPoint) it.next();
...
/*********************************************************************/   
	...
	public void pointChanged(GeoPointInterface P) {
		if (waitingForStartPoint) {
			...
			if (waitingPointSet == null) waitingPointSet = new HashSet<GeoPointInterface>();
