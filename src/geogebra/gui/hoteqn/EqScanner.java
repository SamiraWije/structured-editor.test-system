...
class EqScanner {
   ...
   private Vector<EqToken>    TokenV = new Vector<EqToken> (50,50);        // dynamischer Vector mit allen Tokens
...
public void setEquation(String equation) {
   ...
   // { ... \atop ... }   --> \atop{ ... }{ ... }
   while ( i < TokenV.size() ) {   
      if (TokenV.elementAt(i).typ == EqToken.CHOOSE){

          // einzelnes { suchen
          ii            = i-1; 
          countBeginEnd = 0;
          while ( ii>0 ) {
             if ( TokenV.elementAt(ii).typ == EqToken.EndSym )        countBeginEnd--;
             else if ( TokenV.elementAt(ii).typ == EqToken.BeginSym ) countBeginEnd++;
          ...
          while ( jj < TokenV.size() ) {
             if ( TokenV.elementAt(jj).typ == EqToken.EndSym )        countBeginEnd++;
             else if ( TokenV.elementAt(jj).typ == EqToken.BeginSym ) countBeginEnd--;
             if ( countBeginEnd == 1 ) break; 
             jj++;
          } // end while jj
      ...
      else if ( TokenV.elementAt(i).typ == EqToken.ATOP ){
   ...
   while ( i < TokenV.size()-2 ) {   
      if (TokenV.elementAt(i).typ == EqToken.SQRT){
          if (TokenV.elementAt(i+1).typ == EqToken.Paren) {
             ii             = i+2;  
             countBeginEnd  = 0;
             int countParen = 1;
             while ( ii<TokenV.size() ) {
                ...
                if ( countBeginEnd == 0 ) {
                   if ( TokenV.elementAt(ii).stringS.equals("[") )      countParen++;
                   else if ( TokenV.elementAt(ii).stringS.equals("]") ) countParen--;
                   if ( countParen== 0 ){
...
public EqToken nextToken() {
   ...
   if ( countT >= TokenV.size() ) {
      ...
      return TokenV.elementAt(countT);
   }
} // end nextToken
