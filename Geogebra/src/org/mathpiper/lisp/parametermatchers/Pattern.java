...
public class Pattern {
    ...
    protected List<PatternParameter> iParamMatchers = new ArrayList<PatternParameter>(); //CDeletingArrayGrower<PatternParameter*> iParamMatchers;

    /// List of variables appearing in the pattern.
    protected List<String> iVariables = new ArrayList<String>(); //CArrayGrower<String>

    /// List of predicates which need to be true for a match.
    protected List<ConsPointer> iPredicates = new ArrayList<ConsPointer>(); //CDeletingArrayGrower<ConsPointer[] >
    ...
    public boolean matches(Environment aEnvironment, ConsPointer aArguments) throws Exception {
        ...
        for (i = 0; i < iParamMatchers.size(); i++) {
            ...
            if (!iParamMatchers.get(i).argumentMatches(aEnvironment, argumentsPointer2, argumentsPointer)) {
                return false;
            }
    ...
    public boolean matches(Environment aEnvironment, ConsPointer[] aArguments) throws Exception {
        ...
        for (i = 0; i < iParamMatchers.size(); i++) {
            ...
            PatternParameter patternParameter = iParamMatchers.get(i);
    ...
    protected void setPatternVariables(Environment aEnvironment, ConsPointer[] arguments) throws Exception {
        ...
        for (i = 0; i < iVariables.size(); i++) {
            ...
            aEnvironment.newLocalVariable(iVariables.get(i), arguments[i].getCons());
        }
    }
    ...
    protected boolean checkPredicates(Environment aEnvironment) throws Exception {
        ...
        for (i = 0; i < iPredicates.size(); i++) {
            ConsPointer pred = new ConsPointer();
            aEnvironment.iLispExpressionEvaluator.evaluate(aEnvironment, pred, iPredicates.get(i));
            ...
            if (!isTrue) {
                ...
                strout = Utility.printExpression(iPredicates.get(i), aEnvironment, 60);
    ...
    public List<PatternParameter> getParameterMatchers() {
        return iParamMatchers;
    }

    public List<ConsPointer> getPredicates() {
        return iPredicates;
    }
