...
public class EpsGraphics2D extends java.awt.Graphics2D {
   ...
   */
  private EpsGraphics2D(EpsGraphics2D g) {
   ...
   * like a Graphics2D object. The EPS document is stored in memory.
   */
  private EpsGraphics2D(String title) {
   ...
   */
  private EpsGraphics2D(String title, OutputStream outputStream, int minX,
  ...
  /**
   * Returns the color depth used for all drawing operations. This can be either
  ...
  public void setColor(Color c) {
    ...
    if (getColorDepth() == BLACK_AND_WHITE) {
      ...
      append(c.getRed() / 255f + " " + c.getGreen() / 255f + " " + c.getBlue()
          / 255f + " setrgbcolor");
  ...
  public void setStroke(Stroke s) {
    if (s instanceof BasicStroke) {
      ...
      if (dashArray != null)
        for (float element : dashArray)
          dashes.append(element + " ");
      dashes.append("]");
