...
public class GeoGebraPreferences {
  ...
  /**
   * Breaks up byte array value into pieces and calls prefs.putByteArray(prefs,
   * key+k, piece_k) for every piece.
   */
  ...
  /**
   * Inits factory default XML by taking the preferences XML of this virign
   * application
   */
  ...
  /**
   * Loads the names of the four last used files from the preferences backing
   * store.
   */
  public void loadFileList() {
    System.out.println("Загрузка последних 4 файлов");
    for (int i = 4; i >= 1; i--) {
  ...
  /**
   * Loads XML preferences (empty construction with GUI and kernel settings) and
   * sets application accordingly. This method clears the current construction
   * in the application. Note: the XML string used is the same as for ggb files.
   */
  public void loadXMLPreferences(Application app) {
    app.setWaitCursor();

    System.out.println("load this preferences xml file in application");
    try {
      System.out.println("load tools from ggt file (byte array)");
      byte[] ggtFile = getByteArray(ggbPrefs, TOOLS_FILE_GGT, null);
      app.loadMacroFileFromByteArray(ggtFile, true);

      System.out.println("load preferences xml");
